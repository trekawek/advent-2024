...
fun main() {
    ...
    val antennas = findAntennas(field).flatMap { getAntinodes(it.value, size) }.toSet().toList().sortedBy { it.first }
    println(antennas.size)
}

private fun getAntinodes(antennas: List<Point>, size: Point) = antennas.indices.flatMap { i ->
