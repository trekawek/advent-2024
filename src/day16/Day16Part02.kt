...
fun main() {
    ...
    println(bestPaths.asSequence().map { it.second }.flatMap {
        println(it.charFieldToString())
        it.map { p, c -> if (c == 'O') p else null }
    }.filterNotNull().toSet().count() + 1)
}

fun findBestPaths(
    field: Field<Char>, scores: Field<MutableMap<Direction, Int>>, p: Position, d: Direction, score: Int = 0
    ...
    val allResults = options.flatMap { (d2, s) ->
        val q = p + d2
