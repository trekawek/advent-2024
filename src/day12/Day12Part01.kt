...
fun main() {
    ...
    val visited: List<MutableList<Boolean>> = field.indices.map { field[0].indices.map { false }.toMutableList() }
...
private fun findAreaAndPerimeter(field: List<List<Char?>>, visited: List<MutableList<Boolean>>, p: Position): AreaAndPerimeter {
...
data class AreaAndPerimeter(val area: Int, val perimeter: Int) {
    ...
    operator fun plus(ap: AreaAndPerimeter) = AreaAndPerimeter(area + ap.area, perimeter + ap.perimeter)
