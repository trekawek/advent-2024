...
sealed class AbstractField<E, L : List<E>>(val values: List<L>) {
    ...
    fun find(e: E): Position = map { p, v -> if (v == e) p else null }.values.flatMap { it.filterNotNull() }.first()

    fun <F> map(transform: (p: Position, e: E?) -> F?) = Field(values.mapIndexed { y, row ->
        row.mapIndexed { x, e -> transform(Pair(x, y), e) }
    })

    fun toMutableField() = MutableField(values.map { it.toMutableList() })

    companion object {
        fun readField() = generateSequence(::readlnOrNull).map { it.toList() }.toList().let { Field(it) }
