...
internal fun buildOrder(): Map<Int, Set<Int>> {
    ...
    generateSequence(::readlnOrNull).takeWhile { it.isNotBlank() }
        .map { it.split('|').run { Pair(this[0].toInt(), this[1].toInt()) } }.forEach { p ->
...
internal fun readUpdates() = generateSequence(::readlnOrNull).map { it.split(',').map(String::toInt) }
